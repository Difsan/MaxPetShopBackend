...
public class RouterRestUser {
    ...
    //Doesn'twork yet
    @Bean
    @RouterOperation(path = "/users/byCartId/{userId}", produces = {
            MediaType.APPLICATION_JSON_VALUE},
    ...
    public RouterFunction<ServerResponse> getUserByCartId (GetUserByCartIdUseCase getUserByCartIdUseCase){
        return route(GET("/users/byCartId/{userId}"),
                request -> getUserByCartIdUseCase.apply(request.pathVariable("userId"))
                        .flatMap(product -> ServerResponse.ok()
                                .contentType(MediaType.APPLICATION_JSON)
                                .bodyValue(product))
                        .onErrorResume(throwable -> ServerResponse.status(HttpStatus.NOT_FOUND).bodyValue(throwable.getMessage()))
